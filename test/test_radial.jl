...
@testset "Radial propagation with gradient" begin
import Luna
import Luna: Grid, Maths, PhysData, Nonlinear, Ionisation, NonlinearRHS, Output, Stats, LinearOps, Plotting
import Luna.PhysData: wlfreq
import FFTW
...
nfunω(ω; z=0.0) = nfun(PhysData.wlfreq(ω))
...
responses = (Nonlinear.Kerr_field(PhysData.γ3_gas(gas)),
             Nonlinear.PlasmaCumtrapz(grid.to, grid.to, ionrate, ionpot))
linop = LinearOps.make_linop(grid, q, nfunω)
normfun = NonlinearRHS.norm_radial(grid, q, nfunω)
