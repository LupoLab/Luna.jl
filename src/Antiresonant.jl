...
and keyword arguments other than `wallthickness` (mandatory kwarg) and `loss` (optional,
defaults to `true`) are passed on to the constructor of a [`MarcatilliMode`](@ref).
...
#= If ncl and nco are real, the type of neff depends on whether loss is included.
    Despatching on a value type makes sure the type can be inferred by the compiler =#
__neff(A, B, C, D, σ, nco, loss::Val{true}) = nco*(1 - A*σ^2 - B*σ^3 - C*σ^4 + 1im*D*σ^4)
__neff(A, B, C, D, σ, nco, loss::Val{false}) = nco*(1 - A*σ^2 - B*σ^3 - C*σ^4)
