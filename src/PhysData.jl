...
import SpecialFunctions: dawson
import CoolProp
...
function gauss_glass(material::Symbol)
    if material == :SiO2
        εinf = 0.9918384685601411
        αs = [-0.09802805822457812,
              1.5627376545086484,
              3.772564394238726,
              7.462476811138983,
              0.36248225511105076,
    ...
    return εinf, αs, ω0s, σs
end

n_gauss(material::Symbol, λ) = n_gauss(λ, gauss_glass(material)...)

k_gauss(material::Symbol, λ) = k_gauss(λ, gauss_glass(material)...)

n_gauss(λ, εinf, αs, ω0s, σs) = real(sqrt.(ε_gauss(2e-15*π*c./λ, εinf, αs, ω0s, σs)))

k_gauss(λ, εinf, αs, ω0s, σs) = imag(sqrt.(ε_gauss(2e-15*π*c./λ, εinf, αs, ω0s, σs)))

function ε_gauss(ω, εinf, αs, ω0s, σs)
    ε = Complex(εinf).*one.(ω)
    for i in 1:length(αs)
        ε .+= _gterm.(ω, αs[i], ω0s[i], σs[i])
    end
    return ε
end

function _gterm(ω, α, ω0, σ)
    α*(1im*(Maths.gauss(ω, σ, x0=ω0) - Maths.gauss(ω, σ, x0=-ω0))
       + 2/π * (dawson((ω+ω0)/σ) - dawson((ω-ω0)/σ)))
end
