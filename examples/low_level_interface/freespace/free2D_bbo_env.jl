#=
Here we simulate type I SHG in a BBO crystal driven at 800 nm, as in free2D_bbo.jl,
but using an envelope field. The `thg=true` option to `EnvGrid` is required so that the
second harmonic and the nonlinear product frequencies fit on the (oversampled) grid.
The focus is small and the crystal is reasonably thick, so we observe strong temporal
and spatial walk-off effects.
=#
using Luna
import FFTW
import PyPlot: plt
...
grid = Grid.EnvGrid(thickness, λ0, (250e-9, 2e-6), 120e-15; thg=true)
...
##
responses = (Nonlinear.Chi2Env(θ, ϕ, PhysData.χ2(material), grid.ω0, grid.to),
             Nonlinear.Kerr_env(PhysData.χ3(material)))
...
plt.figure()
plt.subplot(2, 1, 1)
plt.semilogy(ω*1e-12/2π, Iωxy[:, 1, 1]; label="input X")
plt.semilogy(ω*1e-12/2π, Iωxy[:, 2, 1]; label="input Y")
plt.ylim(1e-5mm, 5mm)
plt.xlim(200, 900)
plt.legend()
plt.subplot(2, 1, 2)
plt.semilogy(ω*1e-12/2π, Iωxy[:, 1, end]; label="output X")
plt.semilogy(ω*1e-12/2π, Iωxy[:, 2, end]; label="output Y")
plt.ylim(1e-5mm, 5mm)
plt.xlim(200, 900)
plt.xlabel("Frequency (THz)")
plt.ylabel("SED (arb. u.)")
plt.legend()
